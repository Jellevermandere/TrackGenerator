...
public class FenceGeneration : MonoBehaviour
{
    ...
    public void UpdateRoad()
    {
        ...
        meshFilter.mesh = CreateFenceMesh(terrainGeneration.pointsOnCurve);
        meshFilter.mesh.RecalculateBounds();
        MeshCollider meshCollider = gameObject.GetComponent<MeshCollider>();
        meshCollider.sharedMesh = meshFilter.mesh;

        int textureRepeat = Mathf.RoundToInt(tiling * terrainGeneration.pathLength * 0.05f);
    ...
    public Mesh CreateFenceMesh(Vector3[] points)
    {
        ...
        for (int i = 0; i < points.Length; i++)
        {
            ...
            Vector3 left = Vector3.Normalize(-points[i] + terrainGeneration.worldOffset);
            ...
            float completionPercent = pathDist / terrainGeneration.pathLength; //i / (float)(points.Length - 1);
            ...
            pathDist += terrainGeneration.distanceToNextPoint[i];
            vertIndex += 3;
            triIndex += 12;
        }
