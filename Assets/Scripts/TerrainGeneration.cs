...
public class TerrainGeneration : MonoBehaviour
{
    ...
    public void CreateSpheres()
    {
        ...
        else if(groundPoints.Count > nrOfPoints)
        {
            for (int i = 0; i < groundPoints.Count - nrOfPoints; i++)
            {
                GameObject oldPoint = groundPoints[i + nrOfPoints];
        ...
        {
            groundPoints[i].transform.position = pointsOnCurve[i];
        }
